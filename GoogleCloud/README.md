...
**3.1)** (Alternative Method) If any menus do not work as expected in step 3 above, this is an alternative method to achieve the same result. In JupyterLab, open the terminal by clicking the blue [+] icon near the top left, below File and Edit.  Click the Terminal icon to start a new terminal session.  In the terminal, type the Git command `git clone https://github.com/NIGMS/Introduction-to-Data-Science-for-Biology` and press Enter. This will download the tutorial files into a folder called `Introduction-to-Data-Science-for-Biology`.
